...
module.exports = {
    ...
    async execute(api, event, args) {
        ...
        try {
            const downloadingMessage = await api.sendMessage(global.convertToGothic("Downloading random Shoti video. Please wait..."), threadID, messageID);
            ...
            if (response.data.status) {
                ...
                }, downloadingMessage.messageID, threadID);

                fs.unlinkSync(filePath);
            } else {
                await api.editMessage(global.convertToGothic("Failed to fetch Shoti video. Please try again."), downloadingMessage.messageID, threadID);
            }
        } catch (error) {
            console.error(error);
